...
escher n dib = noneto (esquina n (figura dib)) (lado n (figura dib)) (r270(esquina n (figura dib)))
...
interpBasica True d w h = pictures [line $ triangulo d w h ]
    where  
        triangulo x y z = map (x V.+) [(0,0), y, z, (0,0)]   -- desde Feo.hs
